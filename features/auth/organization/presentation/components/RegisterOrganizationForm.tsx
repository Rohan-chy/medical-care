...
import {
  ...
  useOrganizationRegistrationForm,
} from '../../domain/registerOrganization.schema';
...
import { useRegisterOrganization } from '../../application/usecases/useRegisterOrganization';
...
const OrganizationRegistrationForm = () => {
  ...
  const { mutate: login } = useLogin('Organization');
  const { mutate: register, isPending } = useRegisterOrganization();

  const email = process.env.NEXT_PUBLIC_Organization_SYS_EMAIL;
  const password = process.env.NEXT_PUBLIC_Organization_SYS_PASSWORD;
  ...
  useEffect(() => {
    ...
    if (tenant !== 'Organization' && email && password) {
