...
const DoctorRegistrationForm = () => {
  ...
  // const systemPhone = process.env.NEXT_PUBLIC_Doctor_SYS_EMAIL;
  // const password = process.env.NEXT_PUBLIC_Doctor_SYS_PASSWORD;

  const systemPhone = 'doctor@gmail.com';
  const password = 'Doctor@123';

  // Automatically login on first render
  useEffect(() => {
    if (systemPhone && password) {
      login({ phone: systemPhone, password });
    }
  }, [systemPhone, password, login]);
