...
export default function DoctorHeader({
  ...
  const transformedClinics = useMemo(() => {
    return (
      doctorDetails?.clinicAllocations?.flatMap(
        (clinic) =>
          clinic.branches?.map((branch) => ({
            ...
            dates:
              branch.allocationSchedules?.map((schedule) => ({
                date: schedule.date,
                dayOfWeek: schedule.dayOfWeek,
                slots:
                  schedule.startEndTimes?.map((slot) => ({
                    time: slot.startTime,
  ...
  /* ---------------- SYNC FORM → STATE ---------------- */
  const selectedBranchId = form.watch('clinicId');

  const selectedClinic = transformedClinics?.find(
    (c) => c?.branchId === selectedBranchId
  );
  const selectedDate = selectedClinic?.dates?.[selectedDateIndex] ?? null;

  useEffect(() => {
    if (!transformedClinics?.length) return;

    const firstBranchId = transformedClinics[0]?.branchId;
    // console.log(firstBranchId);

    if (!form.getValues('clinicId')) {
      form.setValue('clinicId', firstBranchId);
      setSelectedDateIndex(0);
    }
  }, [transformedClinics, form]); // ALWAYS same structure

  /* ---------------- OPTIONS ---------------- */
  const clinicOptions = transformedClinics.map((item) => ({
    label: `${item.clinicName} (${item.branchName}) ${item.isMain ? '(Main)' : ''}`,
    value: item.branchId, // IMPORTANT: use branchId
  }));
