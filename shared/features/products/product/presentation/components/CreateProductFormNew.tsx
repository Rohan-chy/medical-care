...
import { useDeleteProductDynamicAttribute } from '../../application/usecases/useDeleteProductDynamicAttribute';
...
const CreateProductFormNew = ({
  ...
  const { mutate: deleteAttribute } = useDeleteProductDynamicAttribute();
                ...
                <div className="rounded-xl border bg-muted/20 p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        append({ id: DEFAULT_UUID, attributeValueId: '' })
                      }
                    >
                      + Add Specs
                    </Button>
                        ...
                        <DeleteAlert
                          onClick={async () => {
                            const fieldData = form.getValues(
                              `dynamicAttributes.${index}`
                            );

                            //  If already exists in DB → delete from API first
                            if (
                              fieldData?.id &&
                              fieldData.id !== DEFAULT_UUID
                            ) {
                              await deleteAttribute({ id: fieldData.id });
                            }
